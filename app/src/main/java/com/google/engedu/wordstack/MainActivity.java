...
//    word2LinearLayout.setOnTouchListener(new TouchListener());
  ...
  public boolean onStartGame(View view) {
    ...
    while (i < count1 && j < count2){
      ...
      if(randomIndex == 0){
        shuffled += word1.charAt(i);
        i++;
      } else {
        shuffled += word2.charAt(j);
        j++;
      }
    }
    ...
    if(i == count1){
      for(int k = j; k < count2; k++){
        shuffled += word2.charAt(k);
      }
    } else {
      for(int k = i; k < count1; k++){
        shuffled += word1.charAt(k);
      }
    }
    messageBox.setText(shuffled);
    for(i = shuffled.length()-1; i >= 0; i--){
      stackedLayout.push(new LetterTile(this, shuffled.charAt(i)));
    }
    /**
     **
     **  YOUR CODE GOES HERE
